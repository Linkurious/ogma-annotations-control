...
/**
 * Helper class for managing comment-arrow lifecycle and relationships
 *
...
export class CommentHelpers {
  constructor(private getState: () => AnnotationState) {}
   ...
   *
   * @param commentPosition - Position for the comment box
  ...
  createCommentWithArrow(
    ...
    const startPoint = this.getTargetPosition(target);
    ...
    arrow.properties.link = {
      start: this.createTargetLink(target, "start"),
      end: {
        id: comment.id,
        side: "end",
        type: "comment",
  ...
  findOrphanedComments(): Comment[] {
    return this.getAllComments().filter(
      (comment) => this.getCommentArrows(comment.id).length === 0
    );
  }
   ...
   */
  private getTargetPosition(target: CommentTarget): Point {
